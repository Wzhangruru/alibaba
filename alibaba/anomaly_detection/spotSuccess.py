...
class biSPOTS:
    ...
    def run(self, with_alarm = True):
        ...
        for i in tqdm.tqdm(range(self.data.size)):
            #if self.extreme_quantile['down']!=None:
            if self.data[i]<self.extreme_quantile['down'] :
                if with_alarm:
                    alarm.append(i)
                else:
    ...
    def plot(self,run_results,i,with_alarm = True):
       ...
       # plt.savefig("./result/0513/304891089933768916891030530437689118916899689933048992300891789198997/metaq-receive-success_rate-"+time1.strftime('%Y-%m-%d')+".png")
        #plt.show()
        #return fig
